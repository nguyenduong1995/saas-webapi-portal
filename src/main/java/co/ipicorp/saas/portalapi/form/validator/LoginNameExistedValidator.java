...
@Component
public class LoginNameExistedValidator extends AbstractFormValidator {
	...
	private boolean validateByEmail(StaffOfCustomerForm staffOfCustomerForm, Errors errors) {
		...
		if ( StringUtils.isNotEmpty(email)) {
		    List<Account> accounts = accountService.getByLoginName(email, Arrays.asList(AccountType.STAFF_OF_CUSTOMER));
    ...
    private boolean isOwnerAccount(Integer staffOfCustomerId, Integer accountId) {
        ...
        if (staffOfCustomerId != null) {
            System.err.println(staffOfCustomerService);
