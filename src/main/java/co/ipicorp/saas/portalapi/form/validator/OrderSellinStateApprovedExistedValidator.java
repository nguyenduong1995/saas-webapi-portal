...
@Component
public class OrderSellinStateApprovedExistedValidator extends AbstractFormValidator {
	...
	private boolean validateById(OrderSellinForm orderSellinForm, Errors errors) {
		...
		if ( orderSellinForm.getOrderSellinId() != null ) {
	    	...
	    	if ( !orderSellin.getOrderStatus().equals(OrderSellinStatus.APPROVED.toString())){
	    		errors.reject(ErrorCode.APP_1909_ORDER_SELLIN_STATE_NOT_EXACTLY,
						new Object[] { "Order Sellin Id", orderSellinForm.getOrderSellinId(), OrderSellinStatus.APPROVED.toString() },
