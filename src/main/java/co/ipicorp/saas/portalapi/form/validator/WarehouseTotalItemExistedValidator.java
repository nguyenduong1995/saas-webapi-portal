...
@Component
public class WarehouseTotalItemExistedValidator extends AbstractFormValidator {
	...
	private boolean validateById(OrderSellinForm orderSellinForm, Errors errors) {
		...
		if ( orderSellinForm.getOrderSellinId() != null ) {
	    	...
	    	if (orderSellinItems == null || orderSellinItems.size() == 0) {
    		...
    		for (OrderSellinItem item : orderSellinItems) {
				...
				WarehouseTotalItem warehouseTotalItem = this.warehouseTotalItemService
																			.getBySellinItemInfo(item.getProductId(), item.getProductVariationId());
