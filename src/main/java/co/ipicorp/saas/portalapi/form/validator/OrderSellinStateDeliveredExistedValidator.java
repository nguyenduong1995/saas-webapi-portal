...
@Component
public class OrderSellinStateDeliveredExistedValidator extends AbstractFormValidator {
	...
	private boolean validateById(OrderSellinForm orderSellinForm, Errors errors) {
		...
		if ( orderSellinForm.getOrderSellinId() != null ) {
	    	...
	    	if ( !orderSellin.getOrderStatus().equals(OrderSellinStatus.DELIVERED.toString())){
	    		errors.reject(ErrorCode.APP_1909_ORDER_SELLIN_STATE_NOT_EXACTLY,
						new Object[] { "Order Sellin Id", orderSellinForm.getOrderSellinId(), OrderSellinStatus.DELIVERED.toString() },
